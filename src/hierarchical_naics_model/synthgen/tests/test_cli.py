...
def test_cli_main_runs_and_writes(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        monkeypatch.setattr(sys, "argv", ["synthgen", "--n", "10", "--out", str(out_path)])
        cli.main()
...
@pytest.mark.parametrize("bad_args", [
    ...
    ["synthgen", "--naics-cut-points", "2", "3", "--naics-branching", "1"],  # wrong branching length
    ["synthgen", "--zip-cut-points", "3", "--zip-branching", "1", "--out", "bad.parquet"],  # minimal branching
])
